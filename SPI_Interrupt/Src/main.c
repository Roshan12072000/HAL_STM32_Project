...
uint8_t tx_data[] = "Hello data from interrpt!";
uint8_t rx_data[50];
    ...
    uart_init();
		...
		W25Q_EraseSector(0x000000);   // erase first sector
		    ...
		    spi_done = 0;
		       ...
		       W25Q_Read_IT(0x000000, rx_data, sizeof(tx_data));
...
void W25Q_Read_IT(uint32_t addr, uint8_t *data, uint16_t len)
{
    ...
    HAL_SPI_Transmit_IT(&hspi1, cmd_buf, 4);
}
...
void HAL_SPI_TxCpltCallback(SPI_HandleTypeDef *hspi)
{
    if (hspi->Instance == SPI1)
    {
        if (spi_stage == 1)
        {
            ...
            spi_stage = 2;
            HAL_SPI_Receive_IT(&hspi1, rx_ptr, rx_len);
        }
    }
}
